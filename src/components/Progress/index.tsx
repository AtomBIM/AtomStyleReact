...
export const Progress = React.forwardRef<HTMLDivElement, ProgressProps>(
  (
    { variant, color, size, value, label, className, barProps, ...rest },
    ref
  ) => {
    // 1. init
    ...
    const progressBarSize = objectsToString(
      sizes[findMatch(valid.sizes, size, "md")]["bar"]
    )
    ...
    const barClasses = twMerge(
      classnames(progressBar, progressVariant),
      barProps?.className
    )
...
Progress.displayName = "AtomStyle.Progress"
