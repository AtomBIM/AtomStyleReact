...
export const Breadcrumbs = forwardRef<HTMLOListElement, BreadcrumbsProps>(
  ({ separator, fullWidth, className, children, ...rest }, ref) => {
    ...
    // 3. set styles
    const breadcrumbsRootClasses = classnames(
      objectsToString(base.root.initial),
      {
    ...
    const breadcrumbsItemClasses = classnames(
      objectsToString(base.item.initial)
    )
    const breadcrumbsSeparatorClasses = classnames(
      objectsToString(base.separator)
    )
              ...
              return (
                <li
                  className={classnames(breadcrumbsItemClasses, {
                    [objectsToString(base.item.disabled)]:
                      child?.props?.disabled,
                  })}
                >
                  {child}
                  {index !== Children.count(children) - 1 && (
                    <span className={breadcrumbsSeparatorClasses}>
                      {separator}
                    </span>
                  )}
                </li>
...
Breadcrumbs.displayName = "AtomStyle.Breadcrumbs"
